...
const MapLeaflet = dynamic(() => import('./MapLeaflet'), {
  ...
  loading: () => (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface)' }}>
      <Loader2 size={28} style={{ color: 'var(--muted)', animation: 'spin 1s linear infinite' }} />
    </div>
  ),
})
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: isDesktop ? '100vh' : '100%', background: '#0A0A0A', overflow: isDesktop ? 'hidden' : 'auto' }}>
      ...
      {!isDesktop && (
        <div style={{ padding: '16px 16px 0', flexShrink: 0 }}>
          <AppHeader eyebrow={new Date().toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' })} title="Mapa" />
        </div>
      )}

      {/* ─── Header + tile switch ─── */}
      <div style={{ padding: isDesktop ? '14px 20px' : '8px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexShrink: 0, flexWrap: 'wrap' }}>
        {isDesktop && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <MapPin size={22} style={{ color: '#D4AF37' }} />
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 900, color: 'var(--cream)', letterSpacing: '-0.5px', lineHeight: 1.1 }}>Mapa de Pedidos</h1>
              <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 1 }}>Visualiza ventas, clientes y oportunidades en el mapa</p>
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          {/* Fecha */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 3, background: 'var(--surface)', borderRadius: 10, padding: '4px 6px', border: '1px solid var(--border)' }}>
            ...
            <select value={fecha} onChange={e => { setPlaying(false); setFecha(e.target.value) }} style={{ background: 'transparent', border: 'none', color: 'var(--cream)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', outline: 'none', minWidth: 92 }}>
              {fechasDisponibles.map(f => <option key={f} value={f} style={{ background: 'var(--surface)' }}>{formatFecha(f)}</option>)}
            </select>
          ...
          <div style={{ display: 'flex', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: 3, gap: 2 }}>
            {TILES.map(t => (
              <button key={t.value} onClick={() => setTileTipo(t.value)} style={{ padding: '7px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 11.5, fontWeight: 700, background: tileTipo === t.value ? '#D4AF37' : 'transparent', color: tileTipo === t.value ? '#1a1200' : 'var(--muted)' }}>{t.label}</button>
            ))}
          </div>
        </div>
      </div>
          ...
          {!loading && !hayDatos && !mostrarSinCompra && (
            <div style={{ position: 'absolute', inset: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
              <div style={{ textAlign: 'center', background: 'rgba(10,10,10,0.92)', borderRadius: 16, padding: '20px 28px', border: '1px solid var(--border)' }}>
                ...
                <p style={{ fontSize: 14, color: 'var(--muted)', fontWeight: 600 }}>Sin ventas para este período</p>
                <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>Selecciona otro día o activa zonas blancas</p>
              </div>
            </div>
          )}
        ...
        <div style={{ width: isDesktop ? 300 : '100%', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 12, overflowY: isDesktop ? 'auto' : 'visible', paddingBottom: isDesktop ? 14 : 0, marginTop: isDesktop ? 0 : 14 }}>
          ...
          <Panel title="Salud del cliente" icon={<Heart size={14} style={{ color: '#5A8A4A' }} />}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0' }}>
              ...
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>Sin información</span>
            </div>
          </Panel>
          ...
          <Panel title="Insights de la zona" icon={<TrendingUp size={14} style={{ color: '#D4AF37' }} />}>
            ...
            {!insights.zonaCaliente && insights.enRiesgo === 0 && (
              <p style={{ fontSize: 12, color: 'var(--muted)' }}>Sin insights para este período.</p>
            )}
          </Panel>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 700 }}>LÍNEA DE TIEMPO</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            <span style={{ fontSize: 10, color: 'var(--muted)' }}>Ventas altas</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 1fr' : '1fr', gap: 14, padding: isDesktop ? '0 20px 20px' : '0 14px 20px', flexShrink: 0 }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--cream)', marginBottom: 12 }}>Resumen por territorio</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {territorios.map(t => {
              const color = VEND_COLOR[t.vendedor] ?? 'var(--muted)'
              const maxL = Math.max(...territorios.map(x => x.litros), 1)
              return (
                <div key={t.vendedor} style={{ background: 'var(--bg)', borderRadius: 10, padding: 12, border: `1px solid ${color}25` }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: color }} />
                    <span style={{ fontSize: 12, color, fontWeight: 700 }}>{dspV(t.vendedor)}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
                    <span style={{ fontSize: 22, fontWeight: 900, color: 'var(--cream)' }}>{formatLitros(t.litros)} L</span>
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>{t.clientes} clientes</span>
                  </div>
        ...
        {/* Ranking de zonas */}
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 16 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--cream)', marginBottom: 12 }}>Ranking de zonas <span style={{ color: 'var(--muted)', fontWeight: 500 }}>(por litros)</span></p>
          {insights.rankingLocalidades.length === 0 ? (
            <p style={{ fontSize: 12, color: 'var(--muted)' }}>Sin datos de zonas.</p>
          ) : (
              ...
              {insights.rankingLocalidades.map(([loc, litros], i) => {
                ...
                return (
                  <div key={loc} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 11, color: 'var(--muted)', width: 14, fontWeight: 700 }}>{i + 1}</span>
                    <span style={{ fontSize: 12.5, color: 'var(--cream)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{loc}</span>
                    <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', fontWeight: 600, width: 64, textAlign: 'right' }}>{formatLitros(litros)} L</span>
...
function navBtn(disabled: boolean): React.CSSProperties {
  return { background: 'none', border: 'none', cursor: disabled ? 'not-allowed' : 'pointer', padding: 4, color: 'var(--muted)', display: 'flex', opacity: disabled ? 0.3 : 1 }
}
...
function KPICard({ icon, color, label, value, alert }: { icon: React.ReactNode; color: string; label: string; value: string | number; alert?: boolean }) {
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderTop: `2px solid ${color}`, borderRadius: 14, padding: '12px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontSize: 10, color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</span>
        <div style={{ color, opacity: 0.8 }}>{icon}</div>
      </div>
      <div style={{ fontSize: 20, fontWeight: 900, color: alert ? color : 'var(--cream)', letterSpacing: '-0.5px', lineHeight: 1 }}>{value}</div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', gap: 9, padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      ...
      <div>
        ...
        <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.45)', lineHeight: 1.35 }}>{body}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '7px 0', background: 'none', border: 'none', cursor: 'pointer' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ width: 9, height: 9, borderRadius: '50%', background: active ? color : 'var(--border)', flexShrink: 0 }} />
        <span style={{ fontSize: 12.5, color: active ? 'var(--cream)' : 'var(--muted)' }}>{label}</span>
      </div>
      {active ? <Eye size={14} style={{ color }} /> : <EyeOff size={14} style={{ color: 'var(--muted)' }} />}
    </button>
  )
}
