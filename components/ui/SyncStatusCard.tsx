...
interface EstadoSync { clientes: EstadoFuente; deudores: EstadoFuente; stock: EstadoFuente; barriles: EstadoFuente; stock_insumos: EstadoFuente }
...
export default function SyncStatusCard({ fuente }: { fuente: 'clientes' | 'deudores' | 'stock' | 'barriles' | 'stock_insumos' }) {
    ...
    <div style={{
      ...
      {!corrida ? (
        ...
        <>
          ...
          {corrida.ok ? (
            <p style={{ fontSize: 12, color: 'var(--muted)' }}>
              {fuente === 'stock' || fuente === 'barriles' || fuente === 'stock_insumos'
                ? `${corrida.total ?? 0} ${fuente === 'barriles' ? 'barriles' : fuente === 'stock_insumos' ? 'insumos' : 'productos'} (foto reemplazada completa)`
                : `${corrida.total ?? 0} procesados`}
              {fuente !== 'stock' && fuente !== 'barriles' && fuente !== 'stock_insumos' && corrida.insertados != null && ` · ${corrida.insertados} nuevos`}
