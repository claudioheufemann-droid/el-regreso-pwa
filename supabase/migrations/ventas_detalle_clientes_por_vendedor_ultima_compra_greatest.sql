-- Pedido de Claudio (10-sep-2026): en "LOCALES" de un vendedor (Venta área
-- comercial), un cliente con pedidos entregados Y un pedido pendiente más
-- reciente seguía mostrando la fecha del entregado como "última compra" —
-- ej. Rausch Restobar con entregas el 8-sep y un pedido pendiente del
-- 10-sep salía fechado "8 Sep 2026", tapando el pedido más nuevo.
--
-- Causa: ventas_detalle_clientes (la función principal, arreglada en
-- ventas_detalle_clientes_orden_por_hora.sql) usa GREATEST(entregado,
-- pendiente) para quedarse con la fecha real más reciente entre ambos. Esta
-- función hermana -misma lógica, pero acotada a un vendedor- se quedó con
-- COALESCE(entregado, pendiente): como el entregado nunca es NULL cuando
-- hay entregas, COALESCE siempre devuelve esa fecha e ignora un pendiente
-- más nuevo. Nunca se sincronizó cuando se corrigió la principal.
